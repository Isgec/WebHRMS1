...
Namespace SIS.HRM
  ...
  Partial Public Class hrmTransfersHistory
    ...
    Private _CompanyIDHRM_Companies As SIS.HRM.hrmCompanies
    Private _DivisionIDHRM_Divisions As SIS.HRM.hrmDivisions
    Private _OfficeIDHRM_Offices As SIS.HRM.hrmOffices
    Private _DepartmentIDHRM_Departments As SIS.HRM.hrmDepartments
    ...
    End Property
		...
		Public ReadOnly Property CardNoHRM_Employees() As SIS.HRM.hrmEmployees
			...
			End Get
		End Property
    Public ReadOnly Property CompanyIDHRM_Companies() As SIS.HRM.hrmCompanies
      Get
        If _CompanyIDHRM_Companies Is Nothing Then
          _CompanyIDHRM_Companies = SIS.HRM.hrmCompanies.GetByID(_CompanyID)
        End If
        Return _CompanyIDHRM_Companies
      End Get
    End Property
    Public ReadOnly Property DivisionIDHRM_Divisions() As SIS.HRM.hrmDivisions
      Get
        If _DivisionIDHRM_Divisions Is Nothing Then
          _DivisionIDHRM_Divisions = SIS.HRM.hrmDivisions.GetByID(_DivisionID)
        End If
        Return _DivisionIDHRM_Divisions
      End Get
    End Property
    Public ReadOnly Property OfficeIDHRM_Offices() As SIS.HRM.hrmOffices
      Get
        If _OfficeIDHRM_Offices Is Nothing Then
          _OfficeIDHRM_Offices = SIS.HRM.hrmOffices.GetByID(_OfficeID)
        End If
        Return _OfficeIDHRM_Offices
      End Get
    End Property
    Public ReadOnly Property DepartmentIDHRM_Departments() As SIS.HRM.hrmDepartments
      Get
        If _DepartmentIDHRM_Departments Is Nothing Then
          _DepartmentIDHRM_Departments = SIS.HRM.hrmDepartments.GetByID(_DepartmentID)
        End If
    ...
    End Function
      'Select By ID One Record Filtered Overloaded GetByID
		<DataObjectMethod(DataObjectMethodType.Select)> _
		Public Shared Function SelectListSite(ByVal startRowIndex As Integer, ByVal maximumRows As Integer, ByVal orderBy As String, ByVal SearchState As Boolean, ByVal SearchText As String) As List(Of SIS.HRM.hrmTransfersHistory)
			Dim Results As List(Of SIS.HRM.hrmTransfersHistory) = Nothing
			Using Con As SqlConnection = New SqlConnection(SIS.SYS.SQLDatabase.DBCommon.GetConnectionString())
				Using Cmd As SqlCommand = Con.CreateCommand()
					Cmd.CommandType = CommandType.StoredProcedure
					If SearchState Then
						Cmd.CommandText = "sphrm_LG_TransfersHistorySelectListSearchSite"
						SIS.SYS.SQLDatabase.DBCommon.AddDBParameter(Cmd, "@KeyWord", SqlDbType.NVarChar, 250, SearchText)
					Else
						Cmd.CommandText = "sphrm_LG_TransfersHistorySelectListFilteresSite"
					End If
    ...
    Public Sub New(ByVal Reader As SqlDataReader)
      ...
      _ActiveState = Ctype(Reader("ActiveState"),Boolean)
			...
			End If
			If Convert.IsDBNull(Reader("VerifiedOn")) Then
				_VerifiedOn = String.Empty
			Else
				_VerifiedOn = CType(Reader("VerifiedOn"), String)
			End If
			_CardNoHRM_Employees = New SIS.HRM.hrmEmployees("HRM_Employees1", Reader)
      _CompanyIDHRM_Companies = New SIS.HRM.hrmCompanies("HRM_Companies2",Reader)
      _DivisionIDHRM_Divisions = New SIS.HRM.hrmDivisions("HRM_Divisions3",Reader)
      _OfficeIDHRM_Offices = New SIS.HRM.hrmOffices("HRM_Offices4",Reader)
      _DepartmentIDHRM_Departments = New SIS.HRM.hrmDepartments("HRM_Departments5",Reader)
      _ProcessedByHRM_Employees = New SIS.HRM.hrmEmployees("HRM_Employees6",Reader)
    End Sub
    Public Sub New(ByVal AliasName As String, ByVal Reader As SqlDataReader)
      ...
      End If
			...
			End If
			If Convert.IsDBNull(Reader(AliasName & "_VerifiedOn")) Then
				_VerifiedOn = String.Empty
			Else
				_VerifiedOn = CType(Reader(AliasName & "_VerifiedOn"), String)
			End If
		End Sub
    Public Sub New()
    End Sub
